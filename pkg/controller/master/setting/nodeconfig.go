...
func (h *Handler) nodeOnChanged(_ string, node *corev1.Node) (*corev1.Node, error) {
	...
	if err != nil {
		if !errors.IsNotFound(err) {
			logrus.Errorf("Get node config with nodeName %s failure: %v", node.Name, err)
			return nil, err
		}
		...
		logrus.Infof("Created new node config for %s", node.Name)
		return node, nil
	}
	...
	if !reflect.DeepEqual(nodeConfig, nodeConfigCpy) {
		if _, err := h.nodeConfigs.Update(nodeConfigCpy); err != nil {
			logrus.Errorf("Update node config with nodeName %s failure: %v", node.Name, err)
			return nil, err
		}
	}
	return node, nil
